...
<template>
  <section id="pricing" style="max-width:1180px; margin:0 auto; padding:clamp(56px,8vw,96px) clamp(16px,4vw,26px);">
    <div style="max-width:42ch; margin-bottom:clamp(32px,5vw,48px);">
      <span style="font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:.16em; color:var(--color-secondary);">{{ $t('prKicker') }}</span>
      <h2 style="margin:10px 0 12px; font-family:var(--font-display); font-weight:800; font-size:clamp(28px,4vw,44px); line-height:1.05; letter-spacing:-.02em; color:var(--color-text); text-wrap:balance;">{{ $t('prTitle') }}</h2>
      <p style="margin:0; font-size:clamp(15px,1.5vw,17px); line-height:1.55; color:var(--color-muted);">{{ $t('prSub') }}</p>
    </div>
    <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr)); gap:18px; align-items:start;">
      <div v-for="(tier, index) in pricing" :key="index" :style="tier.cardStyle">
        <span v-if="tier.rec" style="position:absolute; top:-13px; inset-inline-start:28px; font-size:10.5px; font-weight:700; text-transform:uppercase; letter-spacing:.1em; color:var(--color-on-primary); background:var(--color-primary); padding:6px 13px; border-radius:var(--radius-full); box-shadow:var(--shadow-1);">{{ $t('recommended') }}</span>
        <h3 style="margin:0 0 4px; font-family:var(--font-display); font-weight:700; font-size:19px; color:var(--color-text);">{{ tier.name }}</h3>
        <p style="margin:0 0 18px; font-size:13px; line-height:1.4; color:var(--color-muted);">{{ tier.tagline }}</p>
        <div style="display:flex; align-items:baseline; gap:8px; margin-bottom:6px;">
          <span style="font-family:var(--font-display); font-weight:800; font-size:clamp(34px,4vw,42px); line-height:1; letter-spacing:-.02em; color:var(--color-text);">{{ tier.price }}</span>
        </div>
        <span style="display:block; font-size:12.5px; font-weight:600; color:var(--color-muted); margin-bottom:20px;">{{ tier.period }}</span>
        <a href="#top" :style="tier.ctaStyle">{{ tier.cta }}</a>
        <div style="height:1px; background:var(--color-hairline); margin:22px 0;"></div>
        <ul style="list-style:none; margin:0; padding:0; display:flex; flex-direction:column; gap:11px;">
          <li v-for="(f, i) in tier.features" :key="i" style="display:flex; align-items:flex-start; gap:10px; font-size:13.5px; line-height:1.4; color:var(--color-text);">
            <span style="flex:none; width:18px; height:18px; margin-top:1px; border-radius:50%; background:var(--color-surface-2); display:flex; align-items:center; justify-content:center;">
              <span style="width:7px; height:4px; border-inline-start:2px solid var(--color-success); border-bottom:2px solid var(--color-success); transform:rotate(-45deg); margin-top:-2px;"></span>
            </span>{{ f }}
          </li>
        </ul>
      </div>
