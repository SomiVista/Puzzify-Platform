...
- **Version control discipline:**
  ...
  - Versioning is automated: every merge to `main` publishes a tagged GitHub Release and commits the matching version back (see `.github/workflows/release.yml`). The next version is derived from the released **git tags**, not from `package.json`, so a branch carrying a stale version cannot collide with a published tag. Do **not** bump the version or create tags manually. For a minor/major release, set `package.json` in that PR to a version **above every existing tag** and the automation releases exactly that. The rule lives in `scripts/next-version.mjs` (unit tested).
...
A PR is incomplete until its description is correct and complete. **Gemini generates the first draft via the Auto PR Description workflow**, so accuracy here is a direct responsibility.

- **Follow the template** at `.github/PULL_REQUEST_TEMPLATE.md`. Fill in every section: **What changed**, **Why it changed**, **Technical Summary**, and the **ClickUp Task** link. Output only the templated markdown — no preamble or extra conversational text.
- The **ClickUp Task** link must be real and correctly formatted: `[ClickUp Task](https://app.clickup.com/t/<task_id>)`. Detect the task ID from the branch name or commit messages (alphanumeric like `86...` or starting with `CU-`). If none is found, write exactly `No ClickUp task ID found.`
- Base the description strictly on the actual commit messages and code diff — never invent changes that aren't in the diff.
- The PR author must **review and correct** the generated description before requesting review: confirm it reflects the diff, lists tests added/updated, notes any RTL/i18n or theming impact, and calls out migrations or breaking changes.
